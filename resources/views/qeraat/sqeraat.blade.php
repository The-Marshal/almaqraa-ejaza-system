                            ...
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    ...
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item"><a href="{{URL('/sqeraat')}}">كل الروايات</a></li>
                                        @if(Request::segment(1) == 'sqeraat' AND request()->route('id'))
                                            <li class="breadcrumb-item active">تعديل رواية</li>
                                        @elseif(Request::is('sqeraat/new'))
                                            <li class="breadcrumb-item active">اضافة رواية</li>
                                        @endif
                                        
                                    </ol>
                                </div>
                                <div class="col-md-4 text-end">
                                    <a href="{{route('sqeraat.new')}}" class="btn btn-success">اضافة رواية جديدة</a>
                                </div>
                            </div>
                        </div>
                        <!-- end page title -->
...
@if(Request::segment(1) == 'sqeraat' AND Request::segment(2) == 'show' AND request()->route('id'))
@if(sizeof($data) > 0)
<form action="{{route('sqeraat.update', ['id' => request()->route('id')])}}" method="post" enctype='multipart/form-data'>
    @csrf
                                            <div class="row">
...
<form action="{{route('sqeraat.store')}}" method="post" enctype='multipart/form-data'>
    @csrf
                                            <div class="row">
                    ...
                    <table id="tabletheml" class="table table-hover table-striped table-nowrap mb-0" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$item->name}}</td>
                                <td>
                                    <a href="{{route('sqeraat.show', ['id' => $item->id])}}" class="btn btn-primary btn-sm">تعديل</a> -
                                    <a href="{{route('sqeraat.destroy', ['id' => $item->id])}}" onclick="return confirm('هل انت متأكد')" class="btn btn-danger btn-sm">حذف</a> 
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
