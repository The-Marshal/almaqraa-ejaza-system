                    ...
                    <table id="tabletheml" class="table table-hover table-striped table-nowrap mb-0" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
@if($item->status == 'WAIT' AND $item->teacher_re == 'stop') 
<a href="#" data-bs-toggle="modal" data-bs-target="#ejaza-user-{{$item->id}}" class="btn btn-warning btn-sm">بدء المقابلة</a> 

@elseif($item->teacher_re == 'start') 
{{-- نموذج التعيين كمؤهل (PASS) --}}
                            <form method="POST" action="{{ route('ejazah.shaikh.student.update_status', ['studentId' => $item->id]) }}" class="d-inline me-2">
                                @csrf 
                            ...
                            <form method="POST" action="{{ route('ejazah.shaikh.student.update_status', ['studentId' => $item->id]) }}" class="d-inline">
                                @csrf
                                
                                <input type="hidden" name="status" value="unpass">
